...
interface OnlineResult { id:string; title:string; artist:string; source:string }
...
export function SearchResults(){
  ...
  useEffect(()=>{
    ...
    Promise.allSettled([lyricsService.searchLyrics(query,controller.signal),musicMetadataService.search(query,controller.signal)]).then(([lyrics,metadata])=>{
      const merged:OnlineResult[]=[];
      if(lyrics.status==='fulfilled') for(const item of lyrics.value) merged.push({id:`lr-${item.trackName}-${item.artistName}`,title:item.trackName,artist:item.artistName,source:'LRCLIB'});
      if(metadata.status==='fulfilled') for(const item of metadata.value) merged.push({id:item.id,title:item.title,artist:item.artist,source:'MusicBrainz'});
      const seen=new Set<string>(); setOnline(merged.filter(item=>{const key=`${item.title}|${item.artist}`.toLocaleLowerCase();if(seen.has(key))return false;seen.add(key);return !local.some(song=>`${song.title}|${song.artist}`.toLocaleLowerCase()===key)}).slice(0,15));
  ...
  return <main className="searchPage wrap">
    ...
    <section className="searchGroup"><div className="resultTitle"><h2>เพลงจากฐานข้อมูลออนไลน์</h2><span>ค้นด้วยชื่อเพลงและศิลปิน</span></div>
      {loading?<div className="resultSkeleton">{[1,2,3,4].map(i=><i key={i}/>)}</div>:online.length?<div className="onlineResults">{online.map(item=><Link key={item.id} href={`/lookup/?title=${encodeURIComponent(item.title)}&artist=${encodeURIComponent(item.artist)}`}><span className="onlineMusic">♫</span><div><strong>{item.title}</strong><small>{item.artist}</small></div><span className="availability">ตรวจเนื้อเพลง/คอร์ด</span></Link>)}</div>:searched&&query&&<div className="noOnline"><strong>ไม่พบผลลัพธ์เพิ่มเติม</strong><span>ลองใช้ชื่อเพลงภาษาอังกฤษ ชื่อศิลปินเต็ม หรือชื่อคอร์ด เช่น C/E</span></div>}
    </section>
  </main>;
}
